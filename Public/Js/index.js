...
$(function () {
	...
	$( '#send-btn' ).click( function () {
		...
		$.post(handleUrl, {username : username.val(), content : content.val()}, function(data){
			if(data.status){
				var str  = '<dl class="paper a' + Math.ceil(Math.random()*5) + '"><dt>';
					str += '<span class="username">' + data.username + '</span>';
					str += '<span class="num">No.' + data.id + '</span></dt>';
					str += '<dd class="content">' + data.content + '</dd><dd class="bottom">';
					str += '<span class="time">' + data.time + '</span><a href="" class="close"></a></dd></dl>';
				$( '#main' ).append(str);
				$( '#close' ).click();
			}else{
				alert('发布失败');
